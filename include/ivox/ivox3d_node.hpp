...
struct IVoxNode<PointT, dim>::DistPoint {
    ...
    DistPoint(const double d, PointT p, const int i) : dist(d), point(p), idx(i) {}

    PointT Get() { return point; } // node->GetPoint(idx);
...
// #define INNER_TIMER
...
#endif
    int a_count = 0;
    double max_range_sqr = max_range * max_range; // 改为提前计算平方
    for (const auto& pt : points_) {
        ...
        Eigen::Vector3f d_reduce = pt.getVector3fMap() - point.getVector3fMap();
        if(abs(d_reduce[0]) > max_range && abs(d_reduce[1]) > max_range && abs(d_reduce[2]) > max_range){
            continue;
        }
        double d = d_reduce.squaredNorm(); // 平方和
#ifdef INNER_TIMER
        auto t1 = std::chrono::high_resolution_clock::now();
#endif
        if (d < max_range_sqr) { 
            // dis_points.template emplace_back(DistPoint(d, this, &pt - points_.data())); // &pt - points_.data() 这段代码确实是用来表示 pt 在容器 points_ 中的索引
            dis_points.template emplace_back(DistPoint(d, pt, &pt - points_.data()));
        }
#ifdef INNER_TIMER
...
#endif
    // sort by distance
    // 会非常增加耗时。。。点不多的时候反而增加了时间复杂度， nth_element 时间复杂度为 O(n)
    // 应该是控制dis_points规模，每次都只添加最近的的K个点
    // 如果最近点数量新增大于k，则进行排序，并维持数量
