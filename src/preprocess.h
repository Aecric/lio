...
class Preprocess
{
  ...
  void process(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> msg, PointCloudXYZI::Ptr &pcl_out);
  // void set(bool feat_en, int lid_type, double bld, int pfilt_num);

  static const bool time_list(PointType &x, PointType &y) {return (x.curvature < y.curvature);};
  PointCloudXYZI down_sample(bool _down_sample, float _scan_res_min, float _max_num, float _scan_res_max,
  ...
  private:
  void avia_handler(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> msg);
