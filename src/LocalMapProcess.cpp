...
LocalMapProcess::LocalMapProcess(){
    ...
    // pubGlobalmap= n.advertise<sensor_msgs::PointCloud2>("/Globalmap", 1000, true);ros::NodeHandle &n

    // if(pure_loc){
    //     // load map:
    //     pcl::io::loadPCDFile(map_path, *Global_map_load);
    //     std::cout << "[LocalMapProcess] Global_map_load.size(): " << Global_map_load->size() << std::endl;
    ...
    //     pubGlobalmap.publish(laserCloudmsg);
    // }
}
