...
std::vector<VF(4)> pabcd_pre;
std::vector<double> p_norm_pre;
...
// 在这里，output情况下，不用考虑实时的imu数据输入，根据对imu原始数据的建模可以免去原始数据输入，原理上是通过上一帧imu数据得到这一帧数据
// output模式下，bias是废的，无用的，不参与状态更新，由imu的随机建模代替的bias
...
void h_model_output(state_output &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	double h2_start = omp_get_wtime();
	bool match_in_map = false;
	VF(4) pabcd;
	pabcd.setZero();
	normvec->resize(time_seq[k]);// time_seq[k]代表在第k个时间段内的点云数量，一般为1
	int effect_num_k = 0;
	for (int j = 0; j < time_seq[k]; j++)
	{
		PointType &point_body_j  = feats_down_body->points[idx+j+1];
...

		{
			...
			ivox_->GetClosestPoint(point_world_j, points_near, NUM_MATCH_POINTS, max_match_dist); // 大部分时间在这个上面花费，批量进行最近点寻找

			// cout << "points_near.empty(): " << points_near.empty() << endl;
			// if(points_near.empty()){exit(0);}
			// cout << "omp_get_wtime() - h2_start:" << omp_get_wtime() - h2_start << endl;

			if (points_near.empty() || (points_near.size() < NUM_MATCH_POINTS))
			{
				point_selected_surf[idx+j+1] = false;
			}
			else
			{
				...
				// 调用函数 esti_plane，估计平面方程 pabcd，基于附近的点云 points_near 和阈值 plane_thr 来判断平面是否有效。
				// plane_thr: 平面标准的阈值，越小平面越平坦，若平坦程度大于阈值则认为不可用
				if (esti_plane(pabcd, points_near, plane_thr)) //(planeValid)
				{
					
					// 平面的方程系数 (a, b, c, d) (平面方程 ax+by+cz+d=0)
					// 点(x0, y0, z0)到平面的标准化距离为： distance = |ax0 + by0 + cz0 + d| / sqrt(a^2 + b^2 + c^2)
					// 而pabcd的结果为标准平面参数，有 sqrt(a^2 + b^2 + c^2) = 1
					// pd2 计算的是点到平面的距离
					// 在空间直角坐标系中平面Ax+By+Cz+D=0的法向量为n=(A,B,C)，而它的单位法向量即法向量除以法向量的长度，正负代表方向。
					// D是平面到原点的偏移量，与平面上点的位置有关。它决定了平面与坐标系的相对位置。
					float pd2 = fabs(pabcd(0) * point_world_j.x + pabcd(1) * point_world_j.y + pabcd(2) * point_world_j.z + pabcd(3));
					// V3D norm_vec;
					// M3D Rpf, pf;
					// pf = crossmat_list[idx+j+1];
					...
					// 	pabcd(3) = weight * pabcd(3);
					// }
					// std::cout << "pabcd: " << pabcd.transpose() << std::endl;

					// pd2 计算的是点到平面的距离
					// p_norm是body坐标系下点的模长（点到雷达中心的距离）
					// 这个判断换个角度看是 p_norm / pd2 * pd2 > match_s
					// 这个判断的核心是衡量点的几何特征，尤其是它的空间位置与拟合平面的关系。换句话说：
					// 	如果点距离雷达中心较远（较大 p_norm）且同时离平面较近（较小 pd2），那么它可能更有意义，需要被选中。
					// 	如果点离雷达中心近但离平面远，那么它可能被认为不符合平面匹配的要求，因此不会被选中。
					if (p_norm > match_s * pd2 * pd2)
					{
	...
	effct_feat_num += effect_num_k;
	h2_time += omp_get_wtime() - h2_start;
}
...
void est_reset(){
	...
	// time_seq.clear();
	feats_down_body = PointCloudXYZI::Ptr (new PointCloudXYZI(100000, 1));
	feats_down_world = PointCloudXYZI::Ptr (new PointCloudXYZI(100000, 1));
	// crossmat_list.clear();
	angvel_avr = V3D(0, 0, 0.0);
	acc_avr = V3D(0, 0, 0.0);
